...
def _clamp(score: float) -> float:
    """Clamp a metric score into the canonical 0-1 range."""
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an evaluation judge. Score the answer from 0.0 to 1.0 for "
            "each rubric criterion and return JSON.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}\n"
        )
        ...
        if isinstance(parsed, dict) and isinstance(parsed.get("scores"), dict):
            candidate_scores = parsed["scores"]
        ...
        for criterion in rubric:
            normalized = _normalize_score(candidate_scores.get(criterion))
            scores[criterion] = 0.5 if normalized is None else normalized
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if isinstance(item_scores, dict):
                numeric_scores = [
                    float(value)
                    for value in item_scores.values()
                    if isinstance(value, (int, float))
                ]
                averages.append(_average(numeric_scores))

        overall_avg = _average(averages)
        positional_bias = (
            len(averages) > 1
            and all(averages[0] > other for other in averages[1:])
        )
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_faithfulness": _average(faithfulness_scores),
            "avg_relevance": _average(relevance_scores),
            "avg_completeness": _average(completeness_scores),
            "avg_context_recall": (
                _average(context_recall_scores) if context_recall_scores else None
            ),
            "avg_context_precision": (
                _average(context_precision_scores)
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        new_avg_faithfulness = _average([result.faithfulness for result in new_results])
        new_avg_relevance = _average([result.relevance for result in new_results])
        new_avg_completeness = _average([result.completeness for result in new_results])
        baseline_avg_faithfulness = _average(
            [result.faithfulness for result in baseline_results]
        )
        baseline_avg_relevance = _average(
            [result.relevance for result in baseline_results]
        )
        baseline_avg_completeness = _average(
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            "hallucination": (
                "Improve retrieval grounding and add an answer check that blocks "
                "claims unsupported by the retrieved context"
            ),
            "irrelevant": (
                "Tighten prompt instructions and intent handling so the answer "
                "addresses the user's question directly"
            ),
            "incomplete": (
                "Increase retrieval depth or context window so the model sees all "
                "required evidence before answering"
            ),
            "off_topic": (
                "Add query routing or guardrails to keep the response focused on "
                "the requested task"
            ),
            "refusal": (
                "Review safety and scope rules to reduce unnecessary refusals on "
                "in-scope questions"
            ),
            "unknown": (
                "Audit failed traces manually and refine the failure taxonomy "
        ...
        generic_suggestions = [
            (
                "Review the lowest-scoring examples and add targeted prompt or "
                "few-shot updates for those failure patterns"
            ),
            (
                "Track faithfulness, relevance, and completeness in CI so new "
                "changes are blocked when they regress"
            ),
            (
                "Inspect retrieved chunks for noise and tune chunking, filters, "
                "or reranking to improve evidence quality"
            ),
        ]
